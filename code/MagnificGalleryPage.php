	...
	public function getCMSFields() {
		...
		if ($rootFolder = $this->RootFolder()) {
			...
			$fields->addFieldToTab(
					"Root.Albums", new HeaderField(
					_t("MagnificGalleryPage.ALBUMSNOTSAVED", "You may add albums to your gallery once you have saved the page for the first time."), $headingLevel = "3"
					)
			);
		}
		...
		$fields->addFieldsToTab("Root.Configuration", array(
			$albumEffects = new DropdownField('AlbumEffect', _t('MagnificGalleryPage.ALBUMEFFECT', 'Album Effect'), array_combine(self::listEffects(), self::listEffects())),
			new NumericField('MediaPerPage', _t('MagnificGalleryPage.IMAGESPERPAGE', 'Number of images per page')),
			new LiteralField('FolderUsed', '<div class="message">' . _t('MagnificGalleryPage.FOLDERUSED', 'Images will be saved in : %s', array($this->RootFolder()->Filename)) . '</div>')
		));
...
class MagnificGalleryPage_Controller extends Page_Controller {
	...
	public function PaginatedItems() {
		...
		$list = new PaginatedList($this->CurrentAlbum()->GalleryItems(), $this->request);
	...
	public function rss() {
		$rss = new RSSFeed(
				...
				$title = _t('MagnificGalleryPage.RSS_TITLE', 'Recent images from %s', array(Siteconfig::current_site_config()->Title)), // title for this feed, displayed in RSS readers
				$description = _t('MagnificGalleryPage.RSS_DESCRIPTION', 'Get recents images from our website') // description
		);
